import { Container ,Box, Radio, RadioGroup, HStack, VStack, Text, Image,StatLabel, Stat, StatNumber, StatHelpText, StatArrow, Badge, Progress, Button } from '@chakra-ui/react'
   ...
   return <Container maxW={"container.xl"}>
    {
     ...
     <VStack spacing={"4"} p={"4"} alignItems={"flex-start"}>
        <Text fontSize={"small"} opacity={"0.7"} alignSelf={"center"}>
            Last Updated on {Date(coins.market_data.last_update).split("G")[0]}
        </Text>
      ...
      <Stat>
        ...
        <StatHelpText>
          <StatArrow type={coins.market_data.price_change_percentage_24h > 0?"increase":"decrease"}/> {coins.market_data.price_change_percentage_24h}%
        </StatHelpText>
      </Stat>
      <Box w={"full"} borderWidth={1}>
        ...
        {
            btns.map((i)=>(
              <Button key={i} onClick={()=>switchChartStats(i)} margin={"2"}>{i}</Button>
            ))
        }
      ...
      <Badge fontSize={"2xl"}  >
       ...
       <CustomBar 
        ...
        low={`${coins.market_data.low_24h[currency]}`}
      />
